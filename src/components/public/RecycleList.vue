...
<script>
  /* eslint-disable */
  import CubeLoading from './Loading.vue'
  import { isUndef, throttle } from 'assets/js/util.js'
  ...
  export default {
    ...
    mounted () {
      this.throttleScrollFn = throttle(this._onScroll, 100)
      this.throttleResizeFn = throttle(this._onResize, 100)
      this.$el.addEventListener(EVENT_SCROLL, this.throttleScrollFn)
      window.addEventListener(EVENT_RESIZE, this.throttleResizeFn)
      this.init()
    },
    beforeDestroy () {
      this.$el.removeEventListener(EVENT_SCROLL, this.throttleScrollFn)
      window.removeEventListener(EVENT_RESIZE, this.throttleResizeFn)
    },
    methods: {
      ...
      loadItems(isResize) {
        ...
        window.Promise.all(promiseTasks).then(() => {
          this.updateItemTop()
          this.$nextTick(() => {
            let containerFilled = this.$el.getBoundingClientRect().top + this.heights > document.documentElement.clientHeight
            if (this.autoFill && !containerFilled) {
              this.getItems()
            }
          })
        })
